...
export function fuelUp() {
    ...
    buy(availableAmount($item`all-purpose flower`), $item`soda water`);
    create(availableAmount($item`all-purpose flower`), $item`soda bread`);
...
export function useDefaultFamiliar() {
    if (!get("_bagOfCandy")) {
        ...
        useFamiliar($familiar`miniature hipster`);
    } else if (
...
export function fightSausageIfAble(location: Location, macro: Macro) {
    if (kramcoCheck()) {
        equip($item`off-hand`, $item`Kramco Sausage-o-Matic™`);
